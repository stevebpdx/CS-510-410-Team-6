...
namespace SealTeam6.Core
{
    public class Class1
    {
        ...
        public static void PutFile(FluentFTP.FtpClient session, String local, String remote)
        {

            try{

                session.UploadFile(local, remote);
		...
		public static void PutFiles(FluentFTP.FtpClient session, String directory, List<String> files)
		{
			try
			{
                session.UploadFiles(files, directory);
			}
			catch (Exception e)
			{
				Console.WriteLine("Exception: " + e.Message);
        ...
        public static string DeletesDirectory(FluentFTP.FtpClient fluentSession)
        {
