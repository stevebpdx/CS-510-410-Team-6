...
namespace SealTeam6.Core.Tests
{
    ...
    public class SealTeam6ClientTests
    {
        ...
        [TestMethod()]
        public void DeletesDirectoryTest()
        {

            string remotepath = Class1.DeletesDirectory();
            string localpath = AppDomain.CurrentDomain.SetupInformation.ApplicationBase + remotepath;

			try { File.Delete(localpath); } catch { } // swallow exeption
            try { File.Delete(remotepath); } catch { } // swallow exeption


            Assert.IsFalse(session.DirectoryExists(remotepath));
        ...
        public void PutFileTest()
        {
			...
			try { session.DeleteFile(remotepath); } catch { } // swallow exeption
			File.WriteAllText(localpath, "test1");
			session.DownloadFile(localpath, remotepath);
			try { session.Deletefile(remotepath); } catch { } // swallow exeption

			Core.Class1.PutFile(session, localpath, remotepath);

            Assert.IsTrue(session.FileExists(remotepath));
